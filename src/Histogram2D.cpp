...
std::vector<std::vector<int>> construct2DHistogram(
  ...
  std::vector<std::vector<int>> result(
      bins[1], std::vector<int>(bins[0], 0));
  ...
  const auto notInXLimit = [&](const double& value){return value < xlim[0] || value > xlim[1];};
  const auto notInYLimit = [&](const double& value){return value < ylim[0] || value > ylim[1];};
  ...
  for (int i = 0; i < numberOfElements; i++){
    ...
    const auto xEdge = std::find_if(xEdges.begin(), xEdges.end(), [&](const auto& edge){
      return x_value <= edge;
    });
    assert(xEdge != xEdges.end());
    const auto yEdge = std::find_if(yEdges.begin(), yEdges.end(), [&](const auto& edge){
      return y_value <= edge;
    });
