...
class TopologyRegion {
 public:
  ...
  void computeTopologyWith(const std::vector<System>& systems, int numberOfThreads) const;

  [[nodiscard]] inline const Volume& volume() const noexcept {return *volume_;}
  ...
  [[nodiscard]] constexpr const std::optional<std::string>& sampleOutput() const noexcept {return sampleOutput_;}
 ...
 private:
  ...
  void writeSampleOutput_(const std::vector<PathSample>& data,
                          int index) const;
